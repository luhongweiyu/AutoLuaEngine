...
- 通过 root shell 获取当前进程列表
- 返回结构化进程数组，不直接暴露 `ps` 原始文本
...
- 通过 root shell 查询指定进程详情
- `target` 可以是 PID 字符串或进程名；也可以传 `pid` 或 `name`
- 进程名对应多个 PID 时，返回多个进程对象
