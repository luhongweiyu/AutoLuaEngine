...
test("touch-and-screen-scale", function()
    assert(setScreenScale(1, 360, 640) == true)
    local down = table.pack(touchDown(4, 358, 300))
    sleep(20)
    local moved = touchMove(4, 359, 301)
    local up = table.pack(touchUp(4))
    assert(setScreenScale(0) == true)

    assert(down.n == 0, "touchDown must not return a value")
    assert(moved == true, "touchMove failed")
    assert(up.n == 0, "touchUp must not return a value")
    assert(swipe(716, 602, 718, 604, 32) == true, "swipe failed")
