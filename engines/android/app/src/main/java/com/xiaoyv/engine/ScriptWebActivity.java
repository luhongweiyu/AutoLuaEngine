 ...
 * xiaoyv 桥调用引擎 JSON-RPC。这里不对页面来源、跳转、跨域和桥接能力作安全限制。
 */
public final class ScriptWebActivity extends Activity {
     ...
     * evaluateJavascript 会使 xiaoyv 在首段脚本中不可用，因此优先使用 AndroidX WebKit
    ...
    private static final String PAGE_BRIDGE_SCRIPT = "(function(){"
            + "var bridge=window.xiaoyvNative||window.xiaoyv;"
            + "if(!bridge){return;}"
            + "var previous=window.xiaoyv||{};"
            + "window.xiaoyv={"
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(spec.optString("title", "小鱼精灵"));
     ...
     * 根据脚本配置设置 HTML 窗口。
     *
     * w、h、x、y 全部使用屏幕物理像素，与截图和触摸坐标保持一致。没有传入 w/h 时
     * 保持原来的全屏页面；只要传入任意尺寸，就切换为无背景变暗的定位窗口。指定尺寸
     * 会限制在当前屏幕像素范围内，避免窗口完全超出可操作区域。
     */
    private void configureWindow() {
        boolean sizedWindow = spec.has("w") || spec.has("h");
        ...
        int width = readWindowSize("w", screenWidth);
        int height = readWindowSize("h", screenHeight);
    ...
    private void configureWebView(WebView target) {
        ...
        target.addJavascriptInterface(new ScriptWebJavascriptBridge(), "xiaoyvNative");
     ...
     * evaluateJavascript 更早，页面首段 inline script 也能使用 xiaoyv。
     */
    ...
    private void postMessageToPage(String messageJson) {
        ...
        String script = "(function(){var data=" + payload + ";"
                + "if(window.xiaoyv&&typeof window.xiaoyv.onMessage==='function'){window.xiaoyv.onMessage(data);}"
                + "window.dispatchEvent(new CustomEvent('xiaoyv-message',{detail:data}));"
                + "})();";
    ...
    private void reportEngineCallResult(String callbackId, String resultJson, String errorMessage) {
        ...
        String script = "(function(){if(window.xiaoyv&&typeof window.xiaoyv.onCallResult==='function'){"
                + "window.xiaoyv.onCallResult(" + JSONObject.quote(callbackId == null ? "" : callbackId)
                + "," + result + "," + error + ");}})();";
        runOnUiThread(() -> {
            if (webView != null) {
                webView.evaluateJavascript(script, null);
    ...
    public final class ScriptWebJavascriptBridge {
        ...
        /**
         * 允许 HTML 直接调用引擎 JSON-RPC。结果以 xiaoyv.onCallResult 回调返回页面，
