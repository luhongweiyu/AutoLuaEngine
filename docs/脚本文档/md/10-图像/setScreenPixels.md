...
图片会解码为独立的紧凑 RGBA 点阵，原物理屏幕缓存及其内容不会被修改。图片宽高不得超过
当前物理屏幕，不会自动缩放或裁剪。再次调用本方法会直接替换当前图片屏幕。
...
调用 `restoreScreenPixels()` 可主动还原。脚本正常结束、停止、调用 `exitScript()` 或发生错误时，
引擎也会自动还原并释放图片点阵。替换或还原时，已经开始执行的 native 图像算法可以安全读完
旧点阵；脚本取得的裸点阵地址不能跨再次设置、还原或脚本结束继续使用。
