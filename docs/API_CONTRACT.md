...
- `mkdir` 默认递归创建目录，`chmod` 修改八进制权限位
- 第一版不承诺二进制文件、递归删除、chown
...
### 4.3.5 `m.root.process.*`

说明：

- root 进程能力，提供 PID 查询、进程列表、进程详情和结束进程
- 所有方法都是显式 root 能力，不受 Root 模式开关影响
...
### 4.10 `m.app.install(apkPath, replace)` / `m.installApp(...)`
...
### 4.11 `m.app.uninstall(packageName, keepData)` / `m.uninstallApp(...)`
...
### 4.12 `m.app.disable(packageName)` / `m.disableApp(...)`
...
### 4.13 `m.app.enable(packageName)` / `m.enableApp(...)`
...
### 4.15 `m.file.write(path, content)`
...
### 4.18 `m.file.appDataPath(fileName)`
...
| `m.root.file.*` | 支持，文本、mkdir、chmod 第一版 | 预留 | 受限 |
| `m.root.process.*` | 支持，pidOf/list/info/kill 第一版 | 预留 | 受限 |
