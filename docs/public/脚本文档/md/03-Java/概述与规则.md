...
- `loadApk` 成功返回 `ApkLoader`，失败返回 `nil`；APK 会暂存为私有 `base.apk`，其中的
  `lib/<abi>/*.so` 会一并解出。`loader.loadClass(className)` 成功返回 Java Class，失败返回 `nil`。
- `getExtensionPath` 返回已导入扩展中相对路径对应的私有绝对路径；不存在、越界或未导入时返回 `nil`。
