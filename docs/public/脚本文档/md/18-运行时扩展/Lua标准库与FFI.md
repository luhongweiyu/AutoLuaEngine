...
**方法名称：** Lua 标准库时间/文件接口与实验性受限 FFI。

> **状态：实验性。** 以下 `ffi` 内容尚未纳入稳定兼容承诺；`io`、`os` 等 Lua 5.4 标准库不受此状态影响。
...
`ffi` 当前保留为实验性动态库调用，不是完整 LuaJIT FFI，也尚未纳入稳定兼容承诺；在后续
专门讨论前，不建议生产脚本依赖它。当前实现边界：
