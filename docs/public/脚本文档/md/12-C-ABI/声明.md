...
顶层与子函数表入口如下。当前 `EngineApi::abiVersion` 为 `22`：ABI 18 在顶层尾部追加
`getImGuiApi`，ABI 19 在 `EngineDeviceApi` 尾部追加文本剪贴板，ABI 20 在设备子表追加
`callJson` 并在顶层追加 `findPicAll`，ABI 21 预发布布局再加入 `getYoloApi`；ABI 22 为
第一版实际布局并移除未使用的 `logPrint`。ABI 21 不作为兼容目标，后续正式版本只在表尾追加。

```c
typedef struct EngineApi EngineApi;
typedef struct EngineDeviceApi EngineDeviceApi;
typedef struct EngineImGuiApi EngineImGuiApi;
typedef struct EngineYoloApi EngineYoloApi;
...
ImGui 使用独立子函数表，避免把全部控件和图形方法继续追加到顶层。下列直接导出与
`EngineImGuiApi` 对应成员行为一致：
...
`EngineImGuiApi` 当前 ABI 为 `1`。当前公开包未发布包含完整结构体成员的插件 SDK 头文件；
本章只公开可直接调用的 `engine_imgui*` 导出，直接导出与对应子函数表成员行为一致。公开扩展
不应自行推测或复制函数表布局。

YOLO 同样使用独立子函数表。当前 `EngineYoloApi::abiVersion` 为 `1`，直接导出如下：
...
YOLO C ABI 已存在，但运行时仍是可选目录扩展：需先导入
`yolo/libxiaoyv_yolo.so`，模型和图片使用普通文件路径。Lua 已提供 `m.yolo` 映射；JS、Go
仍未绑定，也没有 Java 或全局 `YoloV5` 包装。Lua 用法见 [YOLO 概述](../20-YOLO/概述.md)。
