...
追加 `getImGuiApi`；当前 `EngineApi::abiVersion` 为 20，版本 19 在 `EngineDeviceApi` 尾部
追加文本剪贴板字段，版本 20 在设备子表追加通用平台 JSON 字段，并在顶层表尾追加
`findPicAll`。下列两个入口返回同一张只读 ImGui 函数表：
...
`EngineImGuiApi` 当前 ABI 为 `1`。全部 `engine_imgui*` 控件、图片、样式和绘图声明由
插件 SDK 头文件提供；直接导出与子函数表成员行为一致。
