...
3. **不修改 Lua 官方源码。**
   使用原版 Lua，通过 C API / JNI / 宿主注册函数扩展能力。
...
Lua print/log.print、native 初始化、任务结果、stop 日志均进入统一 log.drain 缓冲。
```
...
- [x] `touch.tap(x, y)`，优先走无障碍
- [x] `touch.swipe(x1, y1, x2, y2, duration)`
- [x] `key.isAccessibilityEnabled()`、`key.back()`、`key.home()`，走无障碍全局动作
- [x] `screen.capture()`，优先 MediaProjection
- [x] 图片对象句柄管理，当前支持基础句柄、`image.release`、`image.getPixel`、`image.getPixels`
- [ ] `image.findColor(...)`
...
```text
touch.tap / touch.swipe / key.back / key.home 已注册。未开启无障碍服务时返回明确错误。
真实点击需要用户在系统设置中手动开启 AutoLuaEngine 无障碍服务。
screen.capture 已接入 MediaProjection + ImageReader。未授权时返回明确错误；
