...
```text
EngineService(:engine) -> 启动/切换模式时准备 RootShellBridge / RootHelperBridge -> 运行期直接复用常驻执行
```
...
引擎启动或切换 Root 模式时会准备 root 授权和常驻运行层。如果 root 不可用，可以用下面的接口查看 stdout、stderr 和退出码：
