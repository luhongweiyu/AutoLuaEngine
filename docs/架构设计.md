...
m.screen.capture -> 缓冲时间内复用屏幕缓存；缓存未命中时 Root 模式走 root helper，无障碍优先模式走 MediaProjection + ImageReader
m.root.screen.capture -> 缓冲时间内复用屏幕缓存；缓存未命中时显式 root helper，不切换到 MediaProjection
```
