...
App / 悬浮窗 / IDE / 后续插件
        ↓
EngineService / EngineHttpServer 只做进程壳和网络壳
        ↓
NativeEngine.callJson(method, paramsJson)
        ↓
engine/engine_command.cpp
        ↓
Lua HostApi / 后续 JsHostApi / 协议命令入口
        ↓
core/SystemApi
        ↓
...
当前 HTTP JSON-RPC 不再在 Java 层分发具体业务。`EngineHttpServer` 只负责解析 `/jsonrpc`、调用 `NativeEngine.callJson(...)`，再把 native 返回的统一信封转换成 JSON-RPC：

```text
native 成功：{"ok":true,"result":...}
native 失败：{"ok":false,"code":...,"error":"..."}
```

`EngineService` 也只负责 `:engine` 生命周期、读取 App 私有脚本文件、广播运行状态和强停进程；脚本运行、停止、暂停、继续、Root 模式切换都通过同一个 native 命令入口执行。
...
当前项目不直接照搬旧项目代码，但 Android 第一版已经按类似边界落地：
...
脚本状态、日志、截图句柄和停止请求当前都留在 `:engine` 进程内，主进程只通过 Intent、状态广播和本地 JSON-RPC 访问。
...
已在 native Engine 内串行化脚本运行，同一进程内多入口同时运行会返回“已有脚本正在运行”。
...
```text
engines/android/app/src/main/cpp/runtime/lua/host_api.h
engines/android/app/src/main/cpp/runtime/lua/host_api.cpp
engines/android/app/src/main/cpp/runtime/lua/lua_runtime.h
engines/android/app/src/main/cpp/runtime/lua/lua_runtime.cpp
```

脚本运行期通用能力位置：

```text
engines/android/app/src/main/cpp/runtime/common/image_store.h
engines/android/app/src/main/cpp/runtime/common/image_store.cpp
engines/android/app/src/main/cpp/runtime/common/log_buffer.h
engines/android/app/src/main/cpp/runtime/common/log_buffer.cpp
```
...
m.screen.capture -> 20ms 内复用屏幕缓存；缓存未命中时 Root 模式走 root helper，无障碍优先模式走 MediaProjection + ImageReader
m.root.screen.capture -> 20ms 内复用屏幕缓存；缓存未命中时显式 root helper，不切换到 MediaProjection
```
...
第一版已经实现本地 HTTP JSON-RPC：

```text
Android 端监听：127.0.0.1:18380
PC 端访问：adb forward tcp:18380 tcp:18380
健康检查：GET /health
命令入口：POST /jsonrpc
Java 边界：EngineHttpServer 只做网络壳
Native 边界：engine_command.cpp 做统一命令分发
```
