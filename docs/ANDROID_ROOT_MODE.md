...
```text
RootShellBridge -> root 授权检查 -> root-only 执行
```
...
```text
Root 模式 -> root screencap 原始输出；root 不可用或截图失败时直接返回错误
...
当前 root 模式还不是独立 root 引擎。触控和按键复用固定的 root shell：
...
`source = "root-screencap"` 表示本帧走 root 原始截图；`source = "media-projection"` 只会在无障碍优先模式下出现。
...
- root 截图当前仍是 `screencap` 原始输出直通，还不是最终高频找色方案。
...
1. root 截图优化：实现常驻 root worker 或独立 root 进程直取帧，减少每帧创建进程和复制大块 stdout 的开销。
