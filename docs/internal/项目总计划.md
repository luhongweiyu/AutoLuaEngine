...
- App/UI
- 常驻 `:engine` 控制进程，以及 Root/App UID 两种外壳共用的一次性脚本 Worker
- HTTP 控制通讯
- Lua 脚本运行、停止、日志
...
- Android Lua 兼容层：加密、HTTP/WebSocket、LuaSocket 请求、FFI、坐标缩放、图色、
  OpenCV、设备文件工具和无障碍节点
- OCR C ABI：预设/自定义 RapidOCR ONNX 模型加载、释放、识别、找文字
...
- 设备 C ABI：应用状态、硬件信息、显示信息、应用管理、Root exec、系统控制和文本剪贴板
...
- VSCode 插件：脚本控制、日志、打包和启动独立抓图工具
- Qt 抓图取色器：多标签图片、设备/本机截图、取色、Lua/JS 格式生成、二值化、点阵字库和投影
- PC/Android 共享图像核心：颜色规则、二值化和字形点阵生成；桌面单字形测试复用完整点阵匹配
