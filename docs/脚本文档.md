...
> 当前 native 自动化层正在按新 C ABI 重写。现阶段脚本层只保留 `print`、`m.sleep`、`m.log.print`、`m.capture`、`m.rootCapture`、`m.keepCapture`、`m.releaseCapture`、`m.setCaptureCacheMs`。截图核心以 [ANDROID_SO_截图核心.md](ANDROID_SO_截图核心.md) 为准；本文旧章节中的设备、文件、触控、按键、图片句柄 API 暂不代表当前可用接口。
...
| 截图 | `m.screen.keepCapture()` | `m.keepCapture()` | [查看](#api-screen-keep-capture) |
| 截图 | `m.screen.releaseCapture()` | `m.releaseCapture()` | [查看](#api-screen-release-capture) |
| 截图 | `m.screen.setCaptureCacheMs(ms)` | `m.setCaptureCacheMs(ms)` | [查看](#api-screen-set-cache-ms) |
...
Root 模式默认开启。引擎启动或切换到 Root 模式时会准备 root 授权和常驻运行层；运行脚本时不重复申请 root。点击、截图等 API 只执行当前模式对应路线，失败时直接返回错误。

```lua
local info = m.device.info()
if info.automationMode == "none" then
    print("需要给 App 授权 root 或开启无障碍")
    return
end
```
...
截图帧由引擎自动管理。默认 20ms 内连续调用 `m.screen.capture()` / `m.capture()` 会复用当前屏幕帧句柄；超过缓冲时间时重新取屏并覆盖旧屏幕帧。调用 `m.keepCapture()` 后会一直复用当前屏幕帧，直到调用 `m.releaseCapture()` 回到按缓冲时间复用。

不要在循环中反复把截图编码成文件再读取。后续找色、比色算法也会基于 native 内存图片句柄实现。

App 内置 `截图压测` 脚本会连续截 10 帧，并输出：
...
- Root 模式下引擎启动或切换模式时准备 root 运行层；运行脚本时不重复申请 root，触控、按键、截图等 API 只执行当前模式对应路线，失败时直接返回错误。
...
获取当前屏幕截图，并返回 native 内存图片句柄。缓冲时间内优先复用当前屏幕帧缓存；缓存未命中时，Root 模式使用 root helper 常驻进程，无障碍优先模式使用 MediaProjection 系统截图授权。
...
- 默认 20ms 内连续截图会复用当前屏幕帧；超过缓冲时间再重新取屏并覆盖旧屏幕帧。
- `source` 和 `captureDurationMs` 用于判断当前实际截图路线和后续压测截图路径。
- App 内置 `截图压测` 脚本可连续验证多帧截图耗时和缓存复用。
- 截图帧由引擎管理，脚本不需要释放。
...
显式 root 截图请求。该接口和普通截图共用屏幕帧缓存；缓存未命中时只走 root helper，root 不可用或截图失败时直接返回错误。
...
- 缓存命中时会直接返回当前缓存帧，`img.source` 可能来自上一帧的实际截图路线。
- 缓存未命中时不受“无障碍优先”模式影响，始终尝试 root 截图。
- 当前通过 root helper 常驻进程取帧；缓冲时间内和普通截图入口共用同一个屏幕帧缓存。

<a id="api-screen-keep-capture"></a>

### 12.3 `m.screen.keepCapture()` / `m.keepCapture()`

开启锁帧缓存。开启后，`m.screen.capture()` 和 `m.root.screen.capture()` 会一直返回当前屏幕缓存帧；如果当前还没有缓存帧，下一次截图会生成一帧并保持复用。
...
```lua
m.keepCapture()
local img1 = m.capture()
m.sleep(100)
local img2 = m.capture()
print(img1.id == img2.id) -- true
```

<a id="api-screen-release-capture"></a>

### 12.4 `m.screen.releaseCapture()` / `m.releaseCapture()`

关闭锁帧缓存，恢复为按缓冲时间复用。它不会强制删除当前帧；如果当前帧仍在缓冲时间内，下一次截图仍可能复用。
...
<a id="api-screen-set-cache-ms"></a>

### 12.5 `m.screen.setCaptureCacheMs(ms)` / `m.setCaptureCacheMs(ms)`

设置截图缓冲时间，单位毫秒。默认是 `20`。传 `0` 表示只在同一毫秒内复用；负数会返回错误。新脚本开始或当前脚本结束后会恢复默认值。

参数：

| 名称 | 类型 | 说明 |
|---|---|---|
| `ms` | number | 缓冲时间，必须大于或等于 0 |
...
`m.screen.capture()` 返回的是当前屏幕帧句柄。这个句柄由引擎自动管理，脚本不需要释放。

规则：

- 缓冲时间内连续调用截图 API，直接返回同一个缓存帧句柄；默认缓冲时间是 20ms。
- 超过缓冲时间后再次调用截图 API，会重新取屏并覆盖旧屏幕帧。
- 调用 `m.keepCapture()` 后始终复用当前屏幕帧；调用 `m.releaseCapture()` 后恢复按缓冲时间复用。
- 脚本结束或新脚本开始时，引擎会清空屏幕帧缓存，并恢复默认 20ms 缓冲时间。
- 普通文件图片、模板图等非屏幕内容不进入这个屏幕帧缓存；后续如实现加载图片，由用户侧单独管理。
...
截图脚本不需要手动释放当前屏幕帧；异常处理只需要围绕脚本业务逻辑本身。
...
| 锁帧缓存 | `m.keepCapture()` / `m.releaseCapture()` | 部分兼容 | 部分兼容 |
| 截图缓冲时间 | `m.setCaptureCacheMs(ms)` | 部分兼容 | 部分兼容 |
...
```text
图片 ID 不存在，或已经被新的屏幕帧覆盖
```

处理：

```text
确认传入的是当前 m.screen.capture 返回的图片对象或 ID；不要长期保存旧屏幕帧句柄
```
