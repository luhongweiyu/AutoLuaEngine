...
> 当前 native 自动化层正在按新 C ABI 重写。现阶段脚本层只保留 `print`、`m.sleep`、`m.log.print`、`m.capture`、`m.keepCapture`、`m.releaseCapture`、`m.setCaptureCacheMs`。截图核心以 [ANDROID_SO_截图核心.md](ANDROID_SO_截图核心.md) 为准；本文旧章节中的设备、文件、触控、按键、图片句柄 API 暂不代表当前可用接口。
