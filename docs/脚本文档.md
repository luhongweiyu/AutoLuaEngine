...
截图帧由引擎自动管理。20ms 内连续调用 `m.screen.capture()` / `m.capture()` 会复用当前屏幕帧句柄；超过 20ms 时重新取屏并覆盖旧屏幕帧。
...
获取当前屏幕截图，并返回 native 内存图片句柄。20ms 内优先复用当前屏幕帧缓存；缓存未命中时，Root 模式使用 root helper 常驻进程，无障碍优先模式使用 MediaProjection 系统截图授权。
...
- 20ms 内连续截图会复用当前屏幕帧；超过 20ms 再重新取屏并覆盖旧屏幕帧。
...
显式 root 截图请求。该接口和普通截图共用 20ms 屏幕帧缓存；缓存未命中时只走 root helper，root 不可用或截图失败时直接返回错误。
...
- 当前通过 root helper 常驻进程取帧；20ms 内和普通截图入口共用同一个屏幕帧缓存。
...
- 20ms 内连续调用截图 API，直接返回同一个缓存帧句柄。
- 超过 20ms 后再次调用截图 API，会重新取屏并覆盖旧屏幕帧。
