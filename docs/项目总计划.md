...
- [x] `m.screen.capture()`，20ms 内复用屏幕缓存；缓存未命中时 Root 模式走 root helper，无障碍优先模式走 MediaProjection
- [x] `m.root.screen.capture()` / `m.rootCapture()`，20ms 内复用屏幕缓存；缓存未命中时显式 root 截图
- [x] 触控和按键复用常驻 root shell，命令失败时直接返回结果
- [x] 屏幕帧缓存管理，当前支持 20ms 截图复用、`m.image.getPixel`、`m.image.getPixels`
- [ ] `m.image.findColor(...)`
